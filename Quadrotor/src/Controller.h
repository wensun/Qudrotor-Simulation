...
public:
	...
	Controller(const std::vector<RRTNode>& Path, const double& d_t, const double& CAR_L, const double& g_r)
	{
		...
		W = identity<Z_DIM>();
		M = identity<U_DIM>() * 0.1*0.1;
		N = identity<Z_DIM>() * 0.01*0.01;
		C = identity<X_DIM>();
		D = identity<U_DIM>();
	}
	...
	bool simulateLQG(const Matrix<X_DIM>& start, const Matrix<X_DIM, X_DIM>& P0, const int& cal_rrt, const int& cal_environment, const Matrix<DIM>& goal)
	{
	...
	bool simulateOneStep(Matrix<X_DIM>& start_true, Matrix<X_DIM>& start, Matrix<X_DIM, X_DIM>& P0, const int& cal_rrt, const int& cal_environment, const Matrix<DIM>& goal){
